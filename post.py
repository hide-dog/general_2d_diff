...
def write_file(outdir,ingrid_dir,innodes,inele,inresult_dir,front,back):
    ...
    for i in range(len(fff)):
        Qval= numpy.loadtxt(fff[i], delimiter=' ', skiprows=1, usecols=(1),unpack=True)
        ...
        write_result(Qval,out_file,outdir)
...
def write_result(Qval,outf,outdir):
    num_cells=len(Qval)
    sss="CELL_DATA "+str(num_cells)+"\nSCALARS cell_scalars float\nLOOKUP_TABLE default\n"
    with open(outdir+"/"+outf,'a') as f:
        f.write(sss)
        for i in range(num_cells):
            f.write(str('{:.7f}'.format(Qval[i])))
            if i != num_cells-1:
                f.write("\n")
